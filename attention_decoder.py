# Copyright 2016 The Tensorow Authors. All Rights Reserved.
...
def attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None, mode=None):
  """
  Args:
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    #during decoding (batch_size = 1) save the encoder states
    if mode is not None and mode == "decode":
        encoder_hstates = encoder_states[0]
#        print(encoder_hstates)
    ...
    def attention(decoder_state, coverage=None):
      ...
      return context_vector, attn_dist, coverage, e

    outputs = []
    # keep track of the states and activations
    per_sentence_states = []
    ...
    per_sentence_cand = []
    per_sentence_cstatinp = []
    per_sentence_cstatefgt = []
    per_sentence_canduw = []
    
    attn_dists = []
    unnormalized_attn_dists = []
    p_gens = []
    state = initial_state
    ...
    if initial_state_attention: # true in decode mode
      ...
      context_vector, _, coverage, _ = attention(initial_state, coverage) # in decode mode, this is what updates the coverage vector
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      cell_output, state, cell_i, cell_f, cell_o, cell_candidate, cstate_inp, c_state_fgt, candidate_uw = cell(x, state) #ht using ht-1
      
      # saving the state vectors, gate activations
      if mode is not None and mode == "decode":
          per_sentence_states.append(state[1]) #LSTMState tuple (c, h)
          per_sentence_cstates.append(state[0]) 
          per_sentence_igates.append(cell_i)
          per_sentence_fgates.append(cell_f)
          per_sentence_ogates.append(cell_o)
          
          per_sentence_cand.append(cell_candidate)
          per_sentence_cstatinp.append(cstate_inp)
          per_sentence_cstatefgt.append(c_state_fgt)
          per_sentence_canduw.append(candidate_uw)
      ...
      if i == 0 and initial_state_attention:  # always true in decode mode
        with variable_scope.variable_scope(variable_scope.get_variable_scope(), reuse=True): # you need this because you've already run the initial attention(...) call
          context_vector, attn_dist, _, unnormalized_attn_dist = attention(state, coverage) # don't allow coverage to update 
      else:
        context_vector, attn_dist, coverage, unnormalized_attn_dist = attention(state, coverage)
      attn_dists.append(attn_dist)
      unnormalized_attn_dists.append(unnormalized_attn_dist) #Leena
    ...
    per_sentence = [per_sentence_states, per_sentence_cstates, per_sentence_igates, per_sentence_fgates, per_sentence_ogates, encoder_hstates, per_sentence_cand, per_sentence_cstatinp, per_sentence_cstatefgt, per_sentence_canduw, unnormalized_attn_dists] #encoder_hstates in place of NOne for decoding
    
    return outputs, state, attn_dists, p_gens, coverage, per_sentence
