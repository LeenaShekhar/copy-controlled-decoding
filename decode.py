...
import sys
sys.path.append("/home/leena/Documents/thesis/pointer-gen/repetition-behavior/myutils/")

import print_statistics
import save_decode_data
...
class BeamSearchDecoder(object):
  ...
  def __init__(self, model, batcher, vocab, lm_model=None, lm_word2idx=None, lm_idx2word=None):
    """Initialize decoder.
    ...
    if FLAGS.single_pass:
      ...
      if FLAGS.pointer_gen_only_vocab:
          ckpt_name = "vocab-ckpt-" + ckpt_path.split('-')[-1] # this is something of the form "ckpt-123456"
...
#          continue
      ...
      all_hyp = beam_search.run_beam_search(self._sess, self._model, self._vocab, batch, counter, self._lm_model, self._lm_word2idx, self._lm_idx2word) #TODO changed the method signature just to look at the outputs of beam search

      if FLAGS.save_values:
          for h in all_hyp:
              output_ids = [int(t) for t in h.tokens[1:]]
              search_str = str(data.outputids2words(output_ids, self._vocab, (batch.art_oovs[0] if FLAGS.pointer_gen else None)))
...
#      hyp_stat += "trigram overlap: %s. bigram overlap: %s. unigram overlap: %s.\n"%(uni, bi, tri)
      ...
      print_statistics.get_overlap(article_withunks.strip(), decoded_output.strip(), match_count=self.overlap_dict)
      hyp_stat += "word overlap: "
      for key, value in self.overlap_dict.iteritems():
          hyp_stat += "\n%d-gram avg overlap: %d"%(key, value/(counter + 1))
          
      # num sentences and avg length
      self.total_nsentence += len(decoded_output.strip().split(".")) #sentences are seperated by "."
      self.total_length += len(decoded_output.strip().split())
      avg_nsentence, avg_length = self.total_nsentence/(counter+1), self.total_length/(counter+1)
      
      hyp_stat += "\nnum sentences: %s. avg len: %s.\n"%(avg_nsentence, avg_length)
      ...
      if FLAGS.save_values:
          save_decode_data.save_data_iteration(self._decode_dir, counter, best_hyp)
      ...
      if FLAGS.single_pass: #change to counter later
          ...
          self.write_for_attnvis(article_withunks, abstract_withunks, decoded_words, best_hyp.attn_dists, best_hyp.p_gens, counter, best_hyp.log_prob, best_hyp.avg_log_prob, best_hyp.average_pgen)#change to counter later
    ...
    if FLAGS.save_values:
        save_decode_data.save_data_once(self._decode_dir, FLAGS.result_path, outut_str, beam_search_str, metadata)
...
#    tf.logging.info("Wrote example %i to file" % ex_index)
  ...
  def write_for_attnvis(self, article, abstract, decoded_words, attn_dists, p_gens, ex_idx, log_prob, avg_log_prob, avg_pgen):
    ...
    if FLAGS.pointer_gen:
      ...
      # Leena: saving more stuff        
      to_write['log_prob'] = log_prob
      to_write['avg_log_prob'] = avg_log_prob
      to_write['avg_pgen'] = avg_pgen
                          
    output_fname =  os.path.join(self._decode_dir, "attn_vis")
    if not os.path.exists(output_fname):
        os.makedirs(output_fname)          
    output_fname = os.path.join(output_fname, '%06d_attn_vis_data.json'%ex_idx)
